...
class WaypointHandler(Node):
    ...
    def data_callback(self, msg: UavData):
        ...
        self.local_pose = msg.local_pose
        self.yaw = msg.yaw
    ...
    def process_waypoint(self, waypoint):
        ...
        while True:
            ...
            if waypoint.type == TYPE_ROTATE:
                ...
                target.yaw_rate = DEFAULT_YAW_RATE
    ...
    def reached_waypoint(self, waypoint) -> bool:
        ...
        if waypoint.type == TYPE_ROTATE:
            ...
            waypoint.type == TYPE_NAVIGATION
            or waypoint.type == TYPE_HORIZONTAL
            or waypoint.type == TYPE_START
        ):
            reached &= (
                abs(waypoint.latitude - self.gps_fix.latitude) <= THRESHOLD_LATITUDE
            )
            reached &= (
                abs(waypoint.longitude - self.gps_fix.longitude) <= THRESHOLD_LONGITUDE
            )
        ...
        if reached:
            ...
            self.get_logger().info(
                f"current {self.gps_fix, self.rel_alt, self.yaw}, \
    ...
    def setup_params(self, raw_wp, target: GlobalPositionTarget):
        ...
        if (
            raw_wp.type == TYPE_TAKEOFF
            or raw_wp.type == TYPE_VERTICAL
            or raw_wp.type == TYPE_LAND
        ):
