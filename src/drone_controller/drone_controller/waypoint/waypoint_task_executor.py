...
class WaypointTaskExecutor(Node):
    ...
    def data_callback(self, msg: UavData):
        ...
        self.panel_callback(msg.panel_pos)
    ...
    def execute_local_clean_task(self, duration=10.0):
        ...
        while rclpy.ok():
            ...
            if not self.panel_detected:
                self.get_logger().info(
                    "panel edge not found, navigate to next waypoint"
                )
                return False
    ...
    def body_move(self, direction, velocity=DEFAULT_BODY_VEL):
        ...
        if direction == BODY_ROTATE_CW or direction == BODY_ROTATE_CCW:
            ...
            point.velocity = Vector3(
                x=coeff_x * velocity, y=coeff_y * velocity, z=coeff_z * velocity
            )
